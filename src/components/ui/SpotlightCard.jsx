...
const PSEUDO_STYLES = `
  ...
  [data-glow]::before {
    background-image: radial-gradient(
      ...
      hsl(var(--hue, 45) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 50) * 1%) / var(--border-spot-opacity, 1)), transparent 100%
    );
    filter: brightness(2);
  }
  [data-glow]::after {
...
export function GlowCard({
  ...
  const inlineStyle = {
    ...
    "--radius": "14",
    "--border": "2",
    "--backdrop": "rgba(212,175,55,0.04)",
    "--backup-border": "rgba(212,175,55,0.2)",
    "--size": "220",
    "--outer": "1",
    ...
    backgroundImage: `radial-gradient(
      ...
      calc(var(--x, 0) * 1px) calc(var(--y, 0) * 1px),
      hsl(var(--hue, 45) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 70) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
    )`,
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: PSEUDO_STYLES }} />
