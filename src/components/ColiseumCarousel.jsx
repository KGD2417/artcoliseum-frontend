...
const DECAY        = 0.90;     // velocity decay per frame
...
export default function ColiseumCarousel({ items = [] }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      cardRefs.current.forEach((card, i) => {
        ...
        const scale      = 1 - t * 0.30;
        const opacity    = visT * (1 - t * 0.60);
        const brightness = 1 - t * 0.30;
    ...
    const tick = (now) => {
      ...
      if (!drag.current.on) {
        ...
        if (absVel < 0.0001 && Math.abs(activeRef.current - Math.round(activeRef.current)) < 0.01) {
    ...
    const onMove = (e) => {
      ...
      if (Math.abs(dx) > 2) { drag.current.moved = true; e.preventDefault(); }
  ...
  return (
    <div ref={stageRef} className="coliseum-carousel" aria-label="Featured artworks">
      ...
      {gallery.map((item, i) => {
        const [fbMedium, fbPrice] = DETAILS[item.text] || ["Original Artwork", "$2,000"];
          ...
          <button
            ...
            <span className="coliseum-carousel-card-face">
              ...
              <span className="coliseum-carousel-card-copy">
                <span className="coliseum-carousel-card-title">{item.text}</span>
                <span className="coliseum-carousel-card-medium">{item.medium || fbMedium}</span>
                <span className="coliseum-carousel-card-price">{item.price || fbPrice}</span>
              </span>
            </span>
