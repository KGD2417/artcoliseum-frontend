...
const TIMELINE = [
  ...
  { state: "pending", label: "DISPATCHED VIA GLOBAL EXPRESS", time: "Estimated in 2 days" },
  { state: "pending", label: "OUT FOR WHITE-GLOVE DELIVERY",  time: "Estimated in 5–6 days" },
  { state: "pending", label: "FINAL INSTALLATION",            time: "Estimated in 7 days" },
];
...
const fmtTrack = () => "AUR-" + Math.floor(1000 + Math.random() * 8999) + "-" + String.fromCharCode(65 + Math.floor(Math.random() * 26)) + String.fromCharCode(65 + Math.floor(Math.random() * 26)) + "X-0" + Math.floor(1 + Math.random() * 8);
...
export default function Checkout() {
  ...
  const [address, setAddress] = useState({
    name: "", phone: "", line1: "", line2: "", city: "", state: "", zip: "", country: "India",
  });
  const [pay, setPay] = useState("upi");
  const [placed, setPlaced] = useState(false);
  const [orderId] = useState(fmtId);
  ...
  const placeOrder = () => {
    if (!items.length || !addressValid) return;
    setPlaced(true);
    ...
    return (
      <section style={{ padding: "100px 24px 80px", maxWidth: 1200, margin: "0 auto" }}>
        <motion.div
          ...
          <div style={{ fontFamily: "'Cinzel',serif", fontSize: 11, letterSpacing: "0.2em", color: "#D4AF37", marginBottom: 10 }}>
            ORDER PLACED
          </div>
          ...
          <p style={{ fontFamily: "'Raleway',sans-serif", fontSize: 14, color: "rgba(200,191,160,0.65)", maxWidth: 540, margin: "0 auto", lineHeight: 1.7 }}>
            Your order <span className="num-value" style={{ color: "#D4AF37" }}>#{orderId}</span> has been confirmed.
            We've sent a confirmation to your registered details and our curator will be in touch within 24 hours.
          </p>
        </motion.div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 36 }} className="ck-grid">
          ...
          <div style={{ padding: "30px 28px", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 12, background: "rgba(255,255,255,0.02)" }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, fontWeight: 700, color: "#fff", marginBottom: 24 }}>
              Order #{orderId}
            </h2>
            ...
            <div style={{ position: "relative", marginBottom: 26 }}>
              {TIMELINE.map((t, i) => (
                <div key={i} style={{ display: "flex", gap: 14, paddingBottom: i !== TIMELINE.length - 1 ? 22 : 0, position: "relative" }}>
                  ...
                  <div>
                    <div style={{ fontFamily: "'Cinzel',serif", fontSize: 10, letterSpacing: "0.16em",
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div className="num-value" style={{ fontFamily: "'Raleway',sans-serif", fontSize: 14, color: "#D4AF37", letterSpacing: "0.1em" }}>{trackingId}</div>
                <button onClick={() => {
            ...
            <button onClick={() => setHistoryOpen(true)}
              style={{ width: "100%", padding: "13px", marginTop: 8,
                ...
                color: "#e8e0d0", fontFamily: "'Cinzel',serif", fontSize: 11, letterSpacing: "0.18em",
                borderRadius: 6, cursor: "pointer" }}>VIEW DETAILED HISTORY</button>
          </div>
          ...
          <div style={{ padding: "30px 28px", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 12, background: "rgba(255,255,255,0.02)", height: "fit-content" }}>
            ...
            {items.map((it) => (
              <div key={it.id} style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "center" }}>
                <SafeImage src={it.img} alt={it.title} style={{ width: 50, height: 50, objectFit: "cover", borderRadius: 4 }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 16, color: "#fff" }}>{it.title}</div>
                  <div style={{ fontFamily: "'Raleway',sans-serif", fontSize: 11, color: "rgba(200,191,160,0.55)" }}>Qty: <span className="num-value">{it.qty}</span></div>
                </div>
                <div className="num-value" style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 16, fontWeight: 600, color: "#D4AF37" }}>
            ...
            <Row label="Delivery" value={<span style={{ color: "#D4AF37" }}>Included</span>} />
            <Row label="Payment" value={PAY_METHODS.find((p) => p.id === pay)?.label} />

            <div style={{ height: 1, background: "rgba(212,175,55,0.18)", margin: "16px 0" }} />

            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 22 }}>
              <span style={{ fontFamily: "'Cinzel',serif", fontSize: 11, letterSpacing: "0.16em", color: "#fff" }}>TOTAL PAID</span>
              <span className="num-value" style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, fontWeight: 700, color: "#D4AF37" }}>{formatPrice(total)}</span>
            </div>

            <div style={{ fontFamily: "'Cinzel',serif", fontSize: 10, letterSpacing: "0.18em", color: "rgba(200,191,160,0.55)", marginBottom: 6 }}>SHIPPING TO</div>
            <div style={{ fontFamily: "'Raleway',sans-serif", fontSize: 13, color: "#e8e0d0", lineHeight: 1.55, marginBottom: 18 }}>
              {address.name}<br />
              {address.line1}{address.line2 ? `, ${address.line2}` : ""}<br />
              {address.city}, {address.state} {address.zip}<br />
              {address.country} · <span className="num-value">{address.phone}</span>
            </div>

            <button onClick={() => navigate("/")} style={{ width: "100%", padding: "13px",
              background: "linear-gradient(135deg,#D4AF37,#e8c53a)", color: "#111",
        ...
        <HistoryModal open={historyOpen} onClose={() => setHistoryOpen(false)} orderId={orderId} trackingId={trackingId} />
      ...
      {items.length === 0 ? (
        <div style={{ padding: 80, textAlign: "center", border: "1px solid rgba(212,175,55,0.18)", borderRadius: 12 }}>
          ...
          <button onClick={() => navigate("/categories")} style={{
            marginTop: 14, padding: "13px 28px", background: "linear-gradient(135deg,#D4AF37,#e8c53a)",
            color: "#111", border: "none", borderRadius: 999, cursor: "pointer",
            fontFamily: "'Cinzel',serif", fontSize: 11, letterSpacing: "0.18em",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 36 }} className="ck-grid">
          ...
          <div>
            ...
            <div style={{ marginBottom: 36 }}>
              {items.map((it, i) => (
                <div key={it.id} style={{
                  display: "flex", gap: 22, alignItems: "center", padding: "20px 0",
                  borderBottom: i !== items.length - 1 ? "1px solid rgba(212,175,55,0.15)" : "none",
                }}>
                  <SafeImage src={it.img} alt={it.title}
                    style={{ width: 86, height: 86, objectFit: "cover", borderRadius: 4, flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "'Cinzel',serif", fontSize: 10, letterSpacing: "0.16em", color: "rgba(200,191,160,0.55)" }}>{(it.artist || "").toUpperCase()}</div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                    <div className="num-value" style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 700, color: "#D4AF37" }}>
                      {formatPrice((it.price || 0) * it.qty)}
                    </div>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 0,
                      border: "1px solid rgba(212,175,55,0.3)", borderRadius: 999, overflow: "hidden" }}>
                      <button onClick={() => setQty(it.id, it.qty - 1)} style={qtyBtn}>−</button>
                      <span className="num-value" style={{
                        minWidth: 30, textAlign: "center", padding: "0 4px",
                        fontFamily: "'Raleway',sans-serif", fontSize: 13, color: "#fff",
                      }}>{it.qty}</span>
                      <button onClick={() => setQty(it.id, it.qty + 1)} style={qtyBtn}>+</button>
                    </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
              {PAY_METHODS.map((m) => (
                <label key={m.id} style={{
                  ...
                  background: pay === m.id ? "rgba(212,175,55,0.08)" : "rgba(255,255,255,0.02)",
                  border: pay === m.id ? "1px solid #D4AF37" : "1px solid rgba(212,175,55,0.18)",
            ...
            {pay === "upi" && (
              <div style={{
                display: "flex", alignItems: "center", gap: 22, padding: 22,
                ...
                <div>
                  <div style={{ fontFamily: "'Cinzel',serif", fontSize: 11, letterSpacing: "0.18em", color: "#D4AF37", marginBottom: 6 }}>SCAN TO PAY</div>
                  <div className="num-value" style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 700, color: "#fff" }}>{formatPrice(total)}</div>
                  <div style={{ fontFamily: "'Raleway',sans-serif", fontSize: 11, color: "rgba(200,191,160,0.6)", marginTop: 6 }}>
                    UPI ID: <span className="num-value" style={{ color: "#D4AF37" }}>aureum@bank</span>
                  </div>
                </div>
              </div>
            )}
            {pay === "card" && (
              <div style={{ padding: 18, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(212,175,55,0.18)", borderRadius: 10, marginBottom: 18 }}>
                <Input label="Card Number"  type="tel" value="" onChange={() => {}} placeholder="•••• •••• •••• ••••" />
                <div style={twoCol}>
                  <Input label="Expiry"     type="tel" value="" onChange={() => {}} placeholder="MM / YY" />
                  <Input label="CVV"        type="tel" value="" onChange={() => {}} placeholder="•••" />
                </div>
              </div>
            )}
            {pay === "cod" && (
              <div style={{ padding: 18, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(212,175,55,0.18)", borderRadius: 10, marginBottom: 18, fontFamily: "'Raleway',sans-serif", fontSize: 13, color: "rgba(200,191,160,0.7)", lineHeight: 1.65 }}>
                Pay our white-glove courier on arrival. We accept cash and contactless cards. A government-issued ID may be requested at delivery.
              </div>
            )}
          </div>
          ...
          <div style={{ padding: "30px 28px", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 12, background: "rgba(255,255,255,0.02)", height: "fit-content", position: "sticky", top: 100 }}>
            ...
            <Row label={`Subtotal (${items.length} ${items.length === 1 ? "piece" : "pieces"})`} value={formatPrice(subtotal)} />
            <Row label="White-glove delivery" value={<span style={{ color: "#D4AF37" }}>Included</span>} />
            ...
            <motion.button
              ...
              style={{
                ...
                background: addressValid ? "linear-gradient(135deg,#D4AF37,#e8c53a)" : "rgba(212,175,55,0.25)",
                color: addressValid ? "#111" : "rgba(255,255,255,0.5)",
                fontFamily: "'Cinzel',serif", fontSize: 12, letterSpacing: "0.2em",
                border: "none", borderRadius: 999, cursor: addressValid ? "pointer" : "not-allowed",
                boxShadow: addressValid ? "0 8px 24px rgba(212,175,55,0.25)" : "none",
              }}>
              PLACE ORDER →
            </motion.button>

            {!addressValid && (
              <div style={{ marginTop: 10, fontFamily: "'Raleway',sans-serif", fontSize: 11, color: "rgba(200,191,160,0.5)", textAlign: "center" }}>
                Fill in delivery address to continue
              </div>
            )}

            <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px dashed rgba(212,175,55,0.2)", fontFamily: "'Raleway',sans-serif", fontSize: 11, color: "rgba(200,191,160,0.55)", lineHeight: 1.65, textAlign: "center" }}>
              Secure checkout · Provenance ledger · 30-day return on undamaged works
            </div>
          </div>
        </div>
      )}
...
function QRPlaceholder({ value }) {
  ...
  const cells = 17;
  const seed = (value || "x").split("").reduce((s, c) => s + c.charCodeAt(0), 0);
  const dots = [];
  ...
  return (
    <div style={{
      width: 130, height: 130, padding: 8, background: "#fff", borderRadius: 8, flexShrink: 0,
...
function HistoryModal({ open, onClose, orderId, trackingId }) {
  const log = [
    { time: "Just now",       title: "Order Confirmed",    detail: `Order #${orderId} received. Payment authorised.` },
    { time: "Within 24 hrs",  title: "Curator Assigned",   detail: "Senior curator will be assigned to oversee crating and shipment." },
    { time: "+1 day",         title: "Condition Report",   detail: "Pre-shipment condition photographs added to digital ledger." },
    { time: "+1 day",         title: "Crating Complete",   detail: "Climate-controlled crate sealed at our vault." },
    { time: "+2 days",        title: "Customs Cleared",    detail: "Export documentation filed and approved." },
    { time: "+2 days",        title: "Dispatched",         detail: `Handed to Aureum Private Logistics. Tracking ID: ${trackingId}.` },
  ];
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          onClick={onClose}
          style={{ position: "fixed", inset: 0, zIndex: 5000, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, overflowY: "auto" }}>
          <motion.div
            onClick={(e) => e.stopPropagation()}
            initial={{ opacity: 0, y: 30, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.96 }}
            transition={{ duration: 0.25 }}
            style={{ background: "#0e0c0a", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 14, padding: "32px 32px 28px", width: "100%", maxWidth: 580 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
              <div>
                ...
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, fontWeight: 700, color: "#fff" }}>Order #{orderId}</div>
              </div>
              <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: "50%", background: "transparent", border: "1px solid rgba(212,175,55,0.25)", color: "#D4AF37", cursor: "pointer", fontSize: 16 }}>×</button>
            </div>
            <div style={{ maxHeight: "60vh", overflowY: "auto", paddingRight: 4 }}>
              {log.map((h, i) => (
                <div key={i} style={{ display: "flex", gap: 14, paddingBottom: i !== log.length - 1 ? 18 : 0, position: "relative" }}>
                  {i !== log.length - 1 && <div style={{ position: "absolute", left: 5, top: 18, bottom: 0, width: 1, background: "rgba(212,175,55,0.2)" }} />}
                  <div style={{ width: 12, height: 12, borderRadius: "50%", background: i === 0 ? "#D4AF37" : "rgba(212,175,55,0.45)", flexShrink: 0, marginTop: 6, boxShadow: i === 0 ? "0 0 0 4px rgba(212,175,55,0.18)" : "none" }} />
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 600, color: "#fff", marginTop: 3 }}>{h.title}</div>
