...
import ImmersiveARStudio from "../components/ImmersiveARStudio";
...
export default function AR() {
  ...
  // This route renders chrome-less (no site navbar). When loaded inside the
  // product page's AR overlay (an iframe), the host already shows a top bar with
  // a Close button — so we hide our own Back control to avoid a duplicate.
  const embedded =
    typeof window !== "undefined" && window.self !== window.top;

  return (
    <div style={{ minHeight: "100vh", padding: embedded ? "32px 24px 80px" : "60px 24px 80px", background: "#080808" }}>
      {/* Back button — hidden when embedded in the product AR overlay */}
      {!embedded && (
        <motion.button
          onClick={() => (window.history.length > 1 ? navigate(-1) : navigate("/"))}
          initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}
          style={{ position: "fixed", top: 20, left: 24, zIndex: 50, display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", background: "rgba(20,14,8,0.85)", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 999, cursor: "pointer", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" }}
      ...
      {mode === "camera"     && <ImmersiveARStudio initialArtworkUrl={initialArtworkUrl} onSwitchToVisualizer={() => setMode("visualizer")} />}
