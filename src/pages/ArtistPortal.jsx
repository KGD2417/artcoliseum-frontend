...
function ArtworkForm({ onPublished }) {
  ...
  const subtypes = cats.filter(
    (c) =>
      c.kind === "subtype" && (!f.category_id || c.parent_id === f.category_id),
  );
  const set = (k) => (e) => setF({ ...f, [k]: e.target.value });
  ...
  const addStyle = async () => {
    if (!newStyle || !f.category_id)
      return alert("Pick a main medium first, then name the style.");
    try {
      const c = await api.artist.addSubtype(newStyle, f.category_id);
      await loadCats();
  ...
  const submit = async () => {
    if (!f.title || !f.category_id)
      return alert("Title and main medium are required.");
    if (f.customizable && !(Number(f.price_per_unit) > 0))
      return alert(
        "Made-to-order artworks need a price per unit greater than 0.",
      );
    if (!f.customizable && !(fixedPrice > 0))
      return alert(
        "Set a price: either a price for the piece, or at least one predefined size with a price.",
      );
    if (images.length === 0)
      return alert("Upload at least one image of your artwork.");
    setBusy(true);
    try {
      await api.artist.createArtwork({
        ...
        depth:
          !f.customizable && is3D && f.depth !== "" ? Number(f.depth) : null,
        ...
        price_per_unit:
          f.customizable && f.price_per_unit ? Number(f.price_per_unit) : null,
        unit: f.customizable ? f.unit : null,
        min_width:
          f.customizable && f.min_width !== "" ? Number(f.min_width) : null,
        max_width:
          f.customizable && f.max_width !== "" ? Number(f.max_width) : null,
        min_height:
          f.customizable && f.min_height !== "" ? Number(f.min_height) : null,
        max_height:
          f.customizable && f.max_height !== "" ? Number(f.max_height) : null,
        min_depth:
          f.customizable && is3D && f.min_depth !== ""
            ? Number(f.min_depth)
            : null,
        max_depth:
          f.customizable && is3D && f.max_depth !== ""
            ? Number(f.max_depth)
            : null,
    ...
    <div style={card}>
      {done && (
        <div
          ...
          ✓ Submitted for review. Your artwork is now{" "}
          <strong>pending admin approval</strong> — it appears in the public
          gallery once approved. Track it under <strong>My Artworks</strong>.
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <input
          ...
          placeholder="Add a new style (e.g. Luminism)"
          value={newStyle}
          onChange={(e) => setNewStyle(e.target.value)}
        />
        <button
          onClick={addStyle}
          style={{ ...btn, padding: "11px 18px", whiteSpace: "nowrap" }}>
          + STYLE
        </button>
      </div>
      ...
      <div
        ...
        {[
          {
            val: false,
            title: "Fixed size & price",
            desc: "A finished piece sold as-is. You set its size and price (one or more sizes).",
          },
          {
            val: true,
            title: "Customize",
            desc: "Buyers choose their own size; the price is calculated from your price per unit.",
          },
        ].map((opt) => {
          ...
          return (
            <button
              key={String(opt.val)}
              type="button"
              onClick={() => setF({ ...f, customizable: opt.val })}
              style={{
                textAlign: "left",
                padding: "14px 16px",
                borderRadius: 12,
              ...
              <div
                ...
                <span
                  style={{
                    ...
                    borderRadius: "50%",
                    flexShrink: 0,
                    border: `2px solid ${active ? gold : "rgba(200,191,160,0.4)"}`,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                  }}>
                ...
                <span
                  style={{
                    ...
                    fontSize: 17,
                    fontWeight: 700,
                    color: active ? "#fff" : "rgba(232,224,208,0.8)",
                  }}>
                  {opt.title}
                </span>
              </div>
      ...
      {f.customizable ? (
        <>
          <label
            ...
            <input
              type="checkbox"
              checked={f.ratio_locked}
              onChange={(e) => setF({ ...f, ratio_locked: e.target.checked })}
          ...
          <div
            ...
            The buyer's total is calculated automatically from the size they
            pick — you don't set a separate price.
          </div>
          <div
            style={{
              fontFamily: "'Cinzel',serif",
          ...
          <div
            ...
            {is3D && (
              <>
                <Field l="MIN DEPTH / LENGTH">
                  <input
                    style={inputStyle}
                    type="number"
                    value={f.min_depth}
                    onChange={set("min_depth")}
                  />
                </Field>
                <Field l="MAX DEPTH / LENGTH">
                  <input
                    style={inputStyle}
                    type="number"
                    value={f.max_depth}
        ...
        <>
          <div
            ...
            ARTWORK SIZE
            {is3D ? " (width × height × depth)" : " (width × height)"}
          </div>
          <div
            style={{
              display: "grid",
          ...
          {predefined.length === 0 ? (
            ...
            <div
              ...
              Price comes from the sizes below — buyers pick one at checkout,
              and the lowest is shown as the “from” price.
            </div>
          )}
          ...
          <div
            ...
            Offering several sizes? Add each with its own price above. Otherwise
            just set the single price.
          </div>
        </>
      )}
      <Field l="IMAGES">
      ...
      <button
        style={{ ...btn, opacity: busy ? 0.7 : 1, marginTop: 8 }}
        disabled={busy}
        onClick={submit}>
...
function PredefinedSizes({ sizes, setSizes }) {
  const add = () =>
    setSizes([
      ...sizes,
      { label: "", width: "", height: "", unit: "cm", price: "" },
    ]);
  const upd = (i, k, v) =>
    setSizes(sizes.map((s, j) => (j === i ? { ...s, [k]: v } : s)));
  return (
      ...
      {sizes.map((s, i) => (
        <div
          ...
          <input
            ...
            placeholder="W"
            value={s.width}
            onChange={(e) => upd(i, "width", e.target.value)}
          />
          <input
            style={{ ...inputStyle, marginBottom: 0 }}
            placeholder="H"
            value={s.height}
            onChange={(e) => upd(i, "height", e.target.value)}
          />
          <input
            style={{ ...inputStyle, marginBottom: 0 }}
            placeholder="Price"
